...
class MsExcelDocumentBackend(DeclarativeDocumentBackend, PaginatedDocumentBackend):
    ...
    def __init__(
        ...
        try:
            ...
            self.instance = CellsApi(os.getenv('CellsCloudClientId'), os.getenv('CellsCloudClientSecret'))

            if isinstance(self.path_or_stream, BytesIO):
                self.workbook = str(uuid.uuid4()) + ".xlsx"
                self.instance.upload_file(UploadFileRequest(self.path_or_stream.getvalue(), self.workbook))
            elif isinstance(self.path_or_stream, Path):
                self.workbook = str(uuid.uuid4()) + Path(str(self.path_or_stream)).suffix
                self.instance.upload_file(UploadFileRequest(str(self.path_or_stream), self.workbook))
    ...
    def page_count(self) -> int:
        if self.is_valid() and self.workbook:
            pagecount = self.instance.get_page_count(GetPageCountRequest(self.workbook) )
